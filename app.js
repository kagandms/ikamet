...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function requestApiKey() {
        return new Promise((resolve, reject) => {
            const apiKey = sessionStorage.getItem('gcv_api_key');
            if (apiKey) {
                resolve(apiKey);
                return;
            }
            ...
            if (!modal || !input || !btn) {
                reject(new Error('API Anahtarı modal bileşenleri bulunamadı.'));
                return;
            }
            ...
            const submitHandler = () => {
                ...
                if (val) {
                    sessionStorage.setItem('gcv_api_key', val);
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    if (response.status === 400 || response.status === 403) {
                        sessionStorage.removeItem('gcv_api_key');
